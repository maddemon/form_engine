...
export const FieldItem: React.FC<FieldItemProps> = React.memo(
  ({ field, isSelected, children, dragListeners, dragAttributes, dragActivatorRef, dragNodeRef, dragStyle }) => {
    ...
    const handleClick = useCallback(
      (e: React.MouseEvent) => {
        if (!isSelected) {
          e.stopPropagation()
          onSelectField(field.id || null)
        }
      },
      [isSelected, onSelectField, field.id],
    )
