...
export function getIcon(item: PaletteItem, token: <K extends keyof ThemeTokens>(key: K) => ThemeTokens[K]): React.ReactNode {
...
function isValidFieldType(type: string): type is import('../../types').FieldType {
  return ALL_FIELD_TYPES.includes(type) || type.startsWith('custom:')
}

let _counter = 0
export function generateFieldId(type: import('../../types').FieldType): string {
  _counter++
  return `field_${type}_${Date.now()}_${_counter}`
}

export function createFieldFromPalette(item: PaletteItem): import('../../types/schema').FormFieldSchema {
  ...
  const base = {
    ...
    type: item.type,
    label: item.label,
    children: [] as import('../../types/schema').FormFieldSchema[],
    ...(item.defaultProps || {}),
  } as import('../../types/schema').FormFieldSchema
  if (item.extraData) {
...
export function getFullPaletteGroups(excludeTypes?: string[]): import('../../types/designer').PaletteGroup[] {
  ...
  if (Object.keys(customGrouped).length > 0) {
    for (const [groupName, configs] of Object.entries(customGrouped)) {
      ...
      const paletteItems = configs
        ...
        .map((config) => ({
          type: config.type as import('../../types').FieldType,
          label: config.label,
