...
describe('useFormValues', () => {
  ...
  it('setFieldValue 合并值', () => {
    const { result } = renderHook(() => useFormValues({ initialValues: { a: 1, b: 2 } }))
  ...
  it('setFieldsValue 批量设值', () => {
    ...
    const { result } = renderHook(() => useFormValues({ initialValues: { a: 1, b: 2 }, onChange }))
  ...
  it('reset 恢复初始值并触发 onChange', () => {
    const onChange = vi.fn()
    const { result } = renderHook(() => useFormValues({ initialValues: { name: 'initial' }, onChange }))
  ...
  it('debouncedOnChange 防抖触发 onChange', () => {
    vi.useFakeTimers()
    const onChange = vi.fn()
    const { result } = renderHook(() => useFormValues({ initialValues: { name: 'test' }, onChange }))
  ...
  it('formValuesRef 始终指向最新值', () => {
    const { result } = renderHook(() => useFormValues({ initialValues: { name: 'initial' } }))
